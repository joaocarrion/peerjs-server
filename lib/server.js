...
app._initializeHTTP = function() {
  ...
  var setForDeletion = function(value) {
    if (!self._deletion) self._deletion = [];
    self._deletion.push(value.id);
  }

  var deleteOld = function() {
    if (self._deletion) {
      self._deletion.forEach(id => {
  ...
  var serializeInfo = function() {
    var info = self._info;
    var ret = [];
    info.forEach(function(value, key, map) {
      //if ((Date.now() - value.transactionInfo.last_timestamp) > 15000)
      // TODO: find a way to remove nodes that have disconnected from the network
      if (false) {
        setForDeletion(value);
      } else {
        var obj = {
          id: value.id,
          connections: [],
  ...
  var removeConnection = function(peer1, id) {
    peer1.connections.delete(id);
    if (peer1.connections.size == 0)
      self._info.delete(peer1.id);
  };

  var handleConnection = function(req, res, next) {
    var current = getId(req.body.id);
    var other = getId(req.body.connectedId);
    var connected = req.body.connected;

    // keeps track of the connections and remove nodes with no connections
    if (connected) {
      current.connections.add(other.id);
      other.connections.add(current.id);
    } else {
      removeConnection(current, other.id);
      removeConnection(other, current.id);
    }

    res.sendStatus(200);
  };

  var handleTransactions = function(req, res, next) {
    var current = getId(req.body.id);
    var info = current.transactionInfo;

    var time = Date.now();
    if ((time - info.last_timestamp) > info.interval) {
      // keep transactions and blocks per interval
      info.last_timestamp = time;
      info.transactions = req.body.transactions;
      info.blocks = req.body.blocks;
    } else {
      info.transactions += req.body.transactions;
      info.blocks += req.body.blocks;
    }
  ...
  this.post('/connection', jsonParser, handleConnection);
  this.post('/transactions', jsonParser, handleTransactions);
};
