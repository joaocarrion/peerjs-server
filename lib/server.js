...
app._initializeHTTP = function() {
  ...
  var serializeInfo = function() {
    removeOldConnections();

    var info = self._info;
    var ret = [];
    info.forEach(function(node, id, map) {
      ret.push(node);
    });
  ...
  var removeOldConnections = function() {
    var markForDeletion = [];
    self._info.forEach(function(node, id, map) {
      if ((Date.now() - node.timestamp) > 15000) {
        markForDeletion.push(id);
      }
    });

    markForDeletion.forEach(id => {
      self._info.delete(id);
    });
  };

  var handleNetwork = function(req, res, next) {
    var node = getId(req.body.id);
    node.timestamp = Date.now();
    node.connections = req.body.connections;
    node.transactions = req.body.transactions;
    node.blocks = req.body.blocks;
  ...
  this.get("/signaling/data/:id", function(req, res, next) {
    var toDelete = [];
    if (!self._messages) {
      self._messages = {};
    }

    if (!self._messages.broadcast) {
      self._messages.broadcast = {};
    }

    // delete old messages
    for (var key in self._messages['broadcast']) {
      if ((Date.now() - self._messages['broadcast'][key].timestamp) > 5000) {
        toDelete.push(key);
      }
    }

    toDelete.forEach(elem => {
      delete self._messages.broadcast[elem];
    });

    // check params
    if (self._messages && req.params && req.params.id) {
      // prepare response array
      console.log("get from: " + req.params.id);
      var retVal = [];
      if (self._messages[req.params.id]) {
        // there are messages for the id
        console.log(JSON.stringify(self._messages[req.params.id], null, 2));
        self._messages[req.params.id].forEach(msg => {
          retVal.push(msg);
        });

        delete self._messages[req.params.id];
      }

      if (self._messages['broadcast']) {
        // there are broadcast messages
        for (var key in self._messages['broadcast']) {
          //console.log("broadcast key: " + key);
          // for each key in broadcast messages, look if not sent to the peer and then add to response
          var msg = self._messages['broadcast'][key];
          //console.log("response broadcast message: " + JSON.stringify(msg, null, 2));
          if (msg.readFrom.indexOf(req.params.id) == -1) {
            //console.log("Not read from id, PUSH to retval");
            retVal.push(msg);
            msg.readFrom.push(req.params.id);
          }
        }
      }

      console.log("Send retval: " + JSON.stringify(retVal));
  ...
  var handleSignaling = function(req, res, next) {
    //console.log("req.body: " + JSON.stringify(req.body, null, 4));
    var msg = req.body.message;
    if (msg.target) {
      if (!self._messages[msg.target]) {
        self._messages[msg.target] = [];
      }

      self._messages[msg.target].push(msg);
    } else if (msg.broadcast) {
      if (!self._broadcastId) {
        self._broadcastId = 0;
        self._messages = {
          broadcast: {},
        };
      ...
      var id = (++self._broadcastId).toString();
      console.log(`id: ${id}, msg: ${JSON.stringify(msg, null, 2)}`);
      console.log(`broadcast: ${JSON.stringify(self._messages['broadcast'])}`);
      self._messages['broadcast'][id] = msg;
      console.log(`broadcast[${id}]: ${JSON.stringify(self._messages['broadcast'][id])}`);
      msg.readFrom = [];
  ...
  this.post('/signaling/data', jsonParser, handleSignaling);

  // post peer information
  this.post('/network', jsonParser, handleNetwork);
  this.get('/network', function(req, res, next) {
    // serialize self._info
    res.setHeader('Content-Type', 'application/json');
    var ret = serializeInfo();
